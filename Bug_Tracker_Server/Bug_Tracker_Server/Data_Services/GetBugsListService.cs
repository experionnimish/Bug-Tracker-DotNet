...
namespace Bug_Tracker_Server.Data_Services
{
    public class GetBugsListService : IGetBugsListService
    {
        ...
        public List<BugReportDto> GetBugsDeveloper(string Type, UserDto User)
        {
            ...
            var bugReportDto = from bugs in entities.bugs
                               from projects in entities.projects
                               from users in entities.users.Where(x => (projects.project_id == bugs.bug_project_id && bugs.bug_developer_id == User.user_id) && (bugs.bug_tester_id == x.user_id || bugs.bug_developer_id == x.user_id || projects.project_manager_id == x.user_id) && (bugs.bug_status == Type || bugs.bug_status == "Resolving"))
                               select new { bugs, projects, users };
        ...
        public List<BugReportDto> GetBugsDeveloperRejected(string Type, UserDto User)
        {
            BugReportDto BugDetails;
            List<BugReportDto> BugDetailsList = new List<BugReportDto>();
            var bugReportDto = from bugs in entities.bugs
                               from projects in entities.projects
                               from users in entities.users.Where(x => (projects.project_id == bugs.bug_project_id && bugs.bug_developer_id == User.user_id) && (bugs.bug_tester_id == x.user_id || bugs.bug_developer_id == x.user_id || projects.project_manager_id == x.user_id) && (bugs.bug_status == "Review Reject" || bugs.bug_status == "Approve Reject"))
                               select new { bugs, projects, users };
